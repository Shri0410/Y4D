...
const Livelihood = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE}/livelihood`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API_BASE}/livelihood`, formData);
      setShowForm(false);
      setFormData({
        title: '',
        description: '',
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        await axios.delete(`${API_BASE}/livelihood/${id}`);
        fetchItems();
        ...
        <div className="container">
          ...
          {!showForm ? (
            ...
            <div className="career-form">
              <h3>Add New Livelihood Program</h3>
