...
router.post('/', authenticateToken, requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(insertQuery, [
      ...
      mobile_number || null, address || null, req.user.id
    ]);

    await db.query(
      `INSERT INTO audit_logs (user_id, action, resource_type, resource_id, details)
       VALUES (?, ?, ?, ?, ?)`,
      [req.user.id, 'create', 'user', result.insertId, `Created user ${username} with role ${role}`]
    );
...
router.patch('/:id/status', authenticateToken, requireRole(['super_admin', 'admin']), async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO audit_logs (user_id, action, resource_type, resource_id, details)
       VALUES (?, ?, ?, ?, ?)`,
      [req.user.id, 'update_status', 'user', id, `Changed status of user ${username} to ${status}`]
    );

    res.json({ message: `User status updated to ${status} successfully`, userId: id, username, status });
  } catch (err) {
...
router.patch('/:id/role', authenticateToken, requireRole(['super_admin']), async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO audit_logs (user_id, action, resource_type, resource_id, details)
       VALUES (?, ?, ?, ?, ?)`,
      [req.user.id, 'update_role', 'user', id, `Changed role of user ${username} to ${role}`]
    );

    res.json({ message: `User role updated to ${role} successfully`, userId: id, username, role });
  } catch (err) {
...
router.delete('/:id', authenticateToken, requireRole(['super_admin']), async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'User deleted successfully', deletedUser: { id, username } });
  } catch (err) {
...
// Debug schema
router.get('/debug/schema', authenticateToken, requireRole(['super_admin', 'admin']), async (req, res) => {
  const testQuery = `
...
// User statistics
router.get('/stats/overview', authenticateToken, requireRole(['super_admin', 'admin']), async (req, res) => {
  try {
    const queries = {
    ...
    const [[total], [approved], [pending], [rejected], [suspended], [byRole]] =
      await Promise.all([
