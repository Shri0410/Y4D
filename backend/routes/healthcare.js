...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO healthcare (title, description, content, image_url, video_url, additional_images, meta_title, meta_description, meta_keywords, is_active, display_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE healthcare SET title = ?, description = ?, content = ?, image_url = ?, video_url = ?, additional_images = ?, meta_title = ?, meta_description = ?, meta_keywords = ?, is_active = ?, display_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
...
router.delete('/:id', async (req, res) => {
  try {
    const [result] = await db.query('DELETE FROM healthcare WHERE id = ?', [req.params.id]);
