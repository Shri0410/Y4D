...
router.get('/', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM livelihood ORDER BY display_order ASC, created_at DESC');
    res.json(rows);
  } catch (err) {
...
router.get('/active', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM livelihood WHERE is_active = TRUE ORDER BY display_order ASC, created_at DESC');
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/:id', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM livelihood WHERE id = ?', [req.params.id]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const [rows] = await db.query('SELECT * FROM livelihood WHERE id = ?', [id]);
    if (rows.length === 0) return res.status(404).json({ error: 'Item not found' });
